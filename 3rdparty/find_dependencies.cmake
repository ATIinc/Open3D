    ...
    if(NOT USE_SYSTEM_FILAMENT)
        ...
        if(BUILD_FILAMENT_FROM_SOURCE)
            ...
            if (UNIX AND NOT APPLE)
                ...
                if(clang_version MATCHES "clang version ([0-9]+)")
                    set(GUESSED_CLANG_LIBDIR "/usr/lib/llvm-${CMAKE_MATCH_1}/lib")
                    find_library(CHECK_CPP_LIB c++ PATHS ${GUESSED_CLANG_LIBDIR} NO_DEFAULT_PATH)
                    if(CHECK_CPP_LIB)
                        set(CLANG_LIBDIR "${GUESSED_CLANG_LIBDIR}")
                    else()
                        message(STATUS "Guessed CLANG_LIBDIR ${GUESSED_CLANG_LIBDIR} does not contain libc++. Will search elsewhere.")
                    endif()
                    unset(CHECK_CPP_LIB CACHE)
                endif()
                if (NOT CLANG_LIBDIR)
                    ...
                    foreach(llvm_ver RANGE 7 19)
                        ...
                        find_library(CPP_LIBRARY_TMP_${llvm_ver}    c++ PATHS ${llvm_lib_dir} NO_DEFAULT_PATH)
                        find_library(CPPABI_LIBRARY_TMP_${llvm_ver} c++abi PATHS ${llvm_lib_dir} NO_DEFAULT_PATH)
                        if (CPP_LIBRARY_TMP_${llvm_ver} AND CPPABI_LIBRARY_TMP_${llvm_ver})
                            set(CLANG_LIBDIR ${llvm_lib_dir})
                            message(STATUS "CLANG_LIBDIR found in ubuntu-default: ${CLANG_LIBDIR}")
                            set(LIBCPP_VERSION ${llvm_ver})
                            set(CPP_LIBRARY ${CPP_LIBRARY_TMP_${llvm_ver}} CACHE FILEPATH "Path to libc++" FORCE)
                            set(CPPABI_LIBRARY ${CPPABI_LIBRARY_TMP_${llvm_ver}} CACHE FILEPATH "Path to libc++abi" FORCE)
                            break()
                        endif()
                    endforeach()
                endif()
        ...
        if(UNIX AND NOT APPLE)
            ...
            if (NOT CLANG_LIBDIR)
                # Loop removed; moved to before filament_build.cmake
            endif()
